...
abstract contract FunctionsBilling is Routable, IFunctionsBilling {
    ...
    uint256 private constant REASONABLE_GAS_PRICE_CEILING =
        1_000_000_000_000_000; // 1 million gwei
    // ================================================================
    ...
    mapping(address transmitter => uint96 balanceJuelsLink)
    ...
    function getWeiPerUnitLink() public view returns (uint256) {
        ...
        (, int256 weiPerUnitLink, , uint256 timestamp, ) = s_linkToNativeFeed
            .latestRoundData();
        // solhint-disable-next-line not-rely-on-time
        if (
            config.feedStalenessSeconds < block.timestamp - timestamp &&
            config.feedStalenessSeconds > 0
        ) {
    ...
    function _calculateCostEstimate(
        ...
        uint256 gasPriceWithOverestimation = gasPriceWei +
            ((gasPriceWei * s_config.fulfillmentGasPriceOverEstimationBP) /
                10_000);
    ...
    function _startBilling(
        ...
        commitment = FunctionsResponse.Commitment({
            ...
            timeoutTimestamp: uint32(
                block.timestamp + config.requestTimeoutSeconds
            ),
            requestId: requestId,
            donFee: donFee,
    ...
    function _fulfillAndBill(
        ...
        bytes
            memory /* offchainMetadata TODO: use in getDonFee() for dynamic billing */
        ...
        (FunctionsResponse.FulfillResult resultCode, uint96 callbackCostJuels) = _getRouter()
            .fulfill(
                response,
                err,
                juelsPerGas,
    ...
    function oracleWithdraw(address recipient, uint96 amount) external {
        ...
        IFunctionsSubscriptions(address(_getRouter())).oracleWithdraw(
            recipient,
            amount
        );
    }
