...
import {IERC20} from "src/vendor/openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "src/vendor/openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
...
abstract contract FunctionsSubscriptions is
    ...
    IERC677Receiver
{
    ...
    mapping(address coordinator => uint96 balanceJuelsLink)
    ...
    mapping(address consumer => mapping(uint64 subscriptionId => Consumer))
        private s_consumers;
    ...
    mapping(bytes32 requestId => bytes32 commitmentHash)
        internal s_requestCommitments;
    ...
    function _markRequestInFlight(
        ...
        s_subscriptions[subscriptionId]
            .blockedBalance += estimatedTotalCostJuels;
    ...
    function _pay(
        ...
        uint96 totalCostJuels = costWithoutCallbackJuels +
            adminFee +
        ...
        return
            Receipt({
                callbackGasCostJuels: callbackGasCostJuels,
                totalCostJuels: totalCostJuels
            });
    }
    ...
    function getSubscriptionsInRange(
        ...
        subscriptions = new Subscription[](
            (subscriptionIdEnd - subscriptionIdStart) + 1
        );
    ...
    function acceptSubscriptionOwnerTransfer(
        ...
        emit SubscriptionOwnerTransferred(
            subscriptionId,
            previousOwner,
            msg.sender
        );
    ...
    function removeConsumer(
        ...
        for (uint256 i = 0; i < consumers.length; ++i) {
            if (consumers[i] == consumer) {
                ...
                s_subscriptions[subscriptionId].consumers[i] = consumers[
                    consumers.length - 1
                ];
    ...
    function addConsumer(
        ...
        if (
            s_subscriptions[subscriptionId].consumers.length >= maximumConsumers
        ) {
    ...
    function _getSubscriptionDepositDetails()
        internal
        virtual
        returns (uint16, uint72);

    function _cancelSubscriptionHelper(
        uint64 subscriptionId,
        address toAddress,
        bool checkDepositRefundability
    ) private {
        ...
        for (uint256 i = 0; i < subscription.consumers.length; ++i) {
            ...
            completedRequests += s_consumers[consumer][subscriptionId]
                .completedRequests;
            delete s_consumers[consumer][subscriptionId];
        }
        ...
        if (
            checkDepositRefundability &&
            completedRequests < subscriptionDepositMinimumRequests
        ) {
            uint96 deposit = subscriptionDepositJuels > balance
                ? balance
                : subscriptionDepositJuels;
            if (deposit > 0) {
    ...
    function pendingRequestExists(
        ...
        // NOTE: loop iterations are bounded by config.maxConsumers
        for (uint256 i = 0; i < consumers.length; ++i) {
            Consumer memory consumer = s_consumers[consumers[i]][
                subscriptionId
            ];
    ...
    function timeoutRequests(
        ...
        for (uint256 i = 0; i < requestsToTimeoutByCommitment.length; ++i) {
            FunctionsResponse.Commitment
                memory request = requestsToTimeoutByCommitment[i];
