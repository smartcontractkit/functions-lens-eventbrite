...
import {Pausable} from "src/vendor/openzeppelin/contracts/utils/Pausable.sol";

contract FunctionsRouter is
    IFunctionsRouter,
    ...
    ConfirmedOwner
{
    ...
    function isValidCallbackGasLimit(
        ...
        uint8 callbackGasLimitsIndexSelector = uint8(
            getFlags(subscriptionId)[MAX_CALLBACK_GAS_LIMIT_FLAGS_INDEX]
        );
        if (
            callbackGasLimitsIndexSelector >=
            s_config.maxCallbackGasLimits.length
        ) {
            revert InvalidGasFlagValue(callbackGasLimitsIndexSelector);
        }
        uint32 maxCallbackGasLimit = s_config.maxCallbackGasLimits[
            callbackGasLimitsIndexSelector
        ];
    ...
    {
        return (
            s_config.subscriptionDepositMinimumRequests,
            s_config.subscriptionDepositJuels
        );
    }
    ...
    function sendRequest(
        ...
        IFunctionsCoordinator coordinator = IFunctionsCoordinator(
            getContractById(donId)
        );
        return
            _sendRequest(
                donId,
    ...
    function _sendRequest(
        ...
        FunctionsResponse.Commitment memory commitment = coordinator
            .startRequest(
                FunctionsResponse.RequestMeta({
                    ...
                    availableBalance: subscription.balance -
                        subscription.blockedBalance,
        ...
        s_requestCommitments[commitment.requestId] = keccak256(
            abi.encode(
                FunctionsResponse.Commitment({
                    ...
                    gasOverheadBeforeCallback: commitment
                        .gasOverheadBeforeCallback,
                    gasOverheadAfterCallback: commitment
    ...
    function fulfill(
        ...
        external
        override
        returns (FunctionsResponse.FulfillResult resultCode, uint96)
    {
        _whenNotPaused();
        ...
        {
            uint96 callbackCost = juelsPerGas *
                SafeCast.toUint96(commitment.callbackGasLimit);
            uint96 totalCostJuels = commitment.adminFee +
                costWithoutCallback +
                callbackCost;
            ...
            if (
                totalCostJuels >
                getSubscription(commitment.subscriptionId).balance
            ) {
                resultCode = FunctionsResponse
                    .FulfillResult
    ...
    function _callback(
        ...
        assembly {
            ...
            success := call(
                ...
                client,
                0,
                add(encodedCallback, 0x20),
                mload(encodedCallback),
                0,
                0
            )
    ...
    function proposeContractsUpdate(
        ...
        if (
            idsArrayLength != proposedContractSetAddresses.length ||
            idsArrayLength > MAX_PROPOSAL_SET_LENGTH
        ) {
    ...
    function _onlySenderThatAcceptedToS() internal view override {
        ...
        if (
            !IAccessController(currentImplementation).hasAccess(
                msg.sender,
                new bytes(0)
            )
        ) {
