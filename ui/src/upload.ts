...
Please make sure your wallet is connected to the Polygon Mumbai testnet, and is funded with Mumbai Matic.
...
You can get some Mumbai MATIC from the Mumbai Faucet: https://mumbaifaucet.com/`
  ...
  const POST_URL = `https://arweave.net/${tx.id}`;
  console.log("Upload done. Post URL:  ", POST_URL);

  return POST_URL;
}

export async function uploadImage(
  file: ILocalFile<MediaImageMimeType>
): Promise<string> {
  const confirm = window.confirm(
    `In this example we will now upload metadata file via the Bundlr Network.
