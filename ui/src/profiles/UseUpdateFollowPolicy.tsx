...
type SupportedFollowPolicy = Exclude<
  FollowPolicyType,
  ...
  return (
    <div>
      <input
        ...
        onChange={() =>
          setSelectedFollowPolicyType(followPolicyType as FollowPolicyType)
        }
        checked={selectedFollowPolicyType === followPolicyType}
      />
      <label htmlFor={followPolicyType}>
        {followPolicyTypeToDescriptionMap[followPolicyType]}
      </label>
    </div>
  );
}
...
function UpdateFollowPolicyForm({
  ...
  const [selectedFollowPolicyType, setSelectedFollowPolicyType] =
    useState<FollowPolicyType | null>(currentFollowPolicy?.type ?? null);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    invariant(
      selectedFollowPolicyType !== null,
      "No follow policy type selected"
    );
    invariant(
      selectedFollowPolicyType !== FollowPolicyType.UNKNOWN,
      "Unknown follow policy type"
    );
  ...
  return (
    ...
    >
      ...
      <button
        disabled={
          currentFollowPolicy?.type === selectedFollowPolicyType || loading
        }
        type="submit"
      >
        <UpdateButtonText
