...
function UpdateProfileForm({ activeProfile }: UpdateProfileFormProps) {
  ...
  const location = activeProfile.metadata?.attributes?.find(
    (a) => a.key === "location"
  );
  const website = activeProfile.metadata?.attributes?.find(
    (a) => a.key === "website"
  );
    ...
    <div>
      ...
      <WhenLoggedIn loadingElement={<Loading />}>
        {({ profile: activeProfile }) => (
          <UpdateProfileForm activeProfile={activeProfile} />
        )}
      </WhenLoggedIn>
